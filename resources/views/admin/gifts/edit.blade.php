...
<div class="main-card mb-3 card">
    <div class="card-body">
        <form class="form-horizontal" action="{{ route('gifts.update',$gift->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('put')
                ...
                <div class="col-sm-6">
                    <select name="gift_category_id" class="form-control">
                        @foreach($categories as $cat)
                            <option value={{$cat->id}} @if($cat->id == $gift->gift_category_id) selected @endif>{{$cat->name}}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                    <input type="number" class="form-control" name="price" placeholder="{{ __('Price') }}" value="{{$gift->price}}">
                    @if ($errors->has('price'))
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                    @if($gift->image != null)
                    <img style="max-width: 300px" class="mw-400 mb-3 show-img img-demo" src="{{ asset('images/gifts/'.$gift->image) }}" alt="">
                    @else
